...
对于包含 $n$ 个叶子节点的Merkle树 $T$，定义：

- 叶子集合：$L = {d_0, d_1, ..., d_{n-1}}$
- 树的高度：$h = \lceil \log_2 n \rceil$
- 节点总数：$2n-1$（完全二叉树）
...
- 当前节点索引：$idx_i = \lfloor m / 2^i \rfloor$
- 兄弟节点索引：$sibling_i = idx_i \oplus 1$（异或运算）
- 方向：$\delta_i = idx_i \bmod 2$
...
$$L' = \text{sort}(L) = {l'_0, l'*1, ..., l'*{n-1}}$$

其中 $l'_0 < l'*1 < ... < l'*{n-1}$（按字典序）

### 4.2 插入位置确定

对于目标数据 $t$，定义插入位置：

$$pos(t) = \max{i : l'_i < H(0x00 \parallel t)} + 1$$

### 4.3 边界证明

不存在性证明包含两个边界证明：

- 左边界：$\pi_L = \text{InclusionProof}(l'_{pos(t)-1})$
- 右边界：$\pi_R = \text{InclusionProof}(l'_{pos(t)})$

### 4.4 验证条件

不存在性验证需满足：

1. $\text{Verify}(\pi_L) = \text{True}$
2. $\text{Verify}(\pi_R) = \text{True}$
3. $l'*{pos(t)-1} < H(0x00 \parallel t) < l'*{pos(t)}$
